...
import {
  importMech, listMechs, deleteMech, selectMech,
  currentMech, formatMechSheet, mutateArmor
  ...
  try {
    ...
    if (sub === "list") {
      ...
      const embed = new EmbedBuilder()
        .setTitle("あなたの機体一覧")
        .setDescription(list.map((m, idx) =>
          `**${idx + 1}. ${m.name}**  (ID:\`${m.id}\`)  Type:${m.type}  機動:${m.mobility}  装甲:${m.armorCurrent}/${m.armorMax}`
        ).join("\n"))
        .setColor(0x2b90d9);

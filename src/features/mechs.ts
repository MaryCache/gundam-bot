...
import { Mech, ImportedMechJSON, MechType, SelectionKey } from "../types.js";
import {
  loadDB, saveDB,
...
export async function importMech(ownerId: string, jsonStr: string): Promise<Mech> {
  let data: ImportedMechJSON;
  try { data = JSON.parse(jsonStr); } catch { throw new Error("JSONのパースに失敗。```で囲って正しいJSONを貼ってね。"); }
  if (!data.機体) throw new Error("キー『機体』がないよ。");

  const src = data.機体;
  const typeRaw = (src.Type ?? "").toString().toUpperCase();
  if (!["F", "S", "E"].includes(typeRaw)) throw new Error("Type は F/S/E のいずれかにしてね。");
  const type = typeRaw as MechType;
...
export async function listMechs(ownerId: string): Promise<Mech[]> {
  const db = await loadDB();
  return db.mechs
...
export function matchMechPropQuery(text: string): string | null {
  const normalized = text.trim();
  const props = ["Type", "TLv", "機動", "装甲", "積載"];
  return props.find(p => normalized === p) ?? null;
}

export async function answerMechProp(sel: SelectionKey, prop: string): Promise<string> {
  const mech = await currentMech(sel);
  if (!mech) return "選択中の機体がないよ！";
  switch (prop) {

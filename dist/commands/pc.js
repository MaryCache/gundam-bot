...
import { importCharacter, listCharacters, deleteCharacter, selectCharacter, currentCharacter, formatSheet } from "../features/characters.js"; // ★ここを .js で
export const pcCommand = new SlashCommandBuilder()
    .setName("pc")
    .setDescription("キャラクター管理")
    .addSubcommand((s) => s.setName("import").setDescription("JSONでキャラ登録")
    .addStringOption((o) => o.setName("json").setDescription("キャラJSON（```不要）").setRequired(true)))
    .addSubcommand((s) => s.setName("list").setDescription("自分のキャラ一覧（ボタン操作）"))
    .addSubcommand((s) => s.setName("select").setDescription("選択キャラ切替")
    ...
    .addSubcommand((s) => s.setName("delete").setDescription("自分のキャラを削除")
    .addStringOption((o) => o.setName("id_or_name").setDescription("キャラIDまたは名前").setRequired(true)))
    .addSubcommand((s) => s.setName("sheet").setDescription("選択キャラのシートを公開"));
export async function handlePc(interaction) {
...
// ====== 一覧メッセージ with ボタン ======
function buildListMessage(chars, ownerId, page, totalPages, pageSize) {
    ...
    const nav = new ActionRowBuilder().addComponents(new ButtonBuilder()
        ...
        .setLabel("次へ ▶")
        .setStyle(ButtonStyle.Secondary)
        .setDisabled(page >= totalPages - 1), new ButtonBuilder()
...
// ====== ボタン操作 ======
export async function handlePcButton(i) {
    ...
    if (action === "delAsk") {
        await i.reply({
            ...
            components: [
                new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`pc:del:${ownerId}:${payload}`).setLabel("削除する").setStyle(ButtonStyle.Danger), new ButtonBuilder().setCustomId(`pc:cancel:${ownerId}:${payload}`).setLabel("キャンセル").setStyle(ButtonStyle.Secondary)),
            ],
            ephemeral: true,
        });
        return true;

...
export async function importCharacter(ownerId, jsonStr) {
    let data;
    try {
        data = JSON.parse(jsonStr);
    }
    catch {
        throw new Error("JSONのパースに失敗しました。```で囲って正しいJSONを貼ってね。");
    }
    if (!data.基本情報 || !data.能力値 || !data.技能)
        throw new Error("必須キー（基本情報/能力値/技能）が不足しています。");
    // 能力値チェック（各1..6）※ 合計≤21の制限は撤廃
    const abilities = data.能力値;
    for (const v of Object.values(abilities)) {
        if (v < 1 || v > 6)
            throw new Error("能力値は各1〜6です。");
    }
    // 技能キーと範囲チェック
    const skills = data.技能;
    for (const k of ALL_SKILLS) {
        if (!(k in skills))
            throw new Error(`技能キーが不足しています：${k}`);
        const v = Number(skills[k]);
        if (v < 0 || v > 10)
            throw new Error(`技能「${k}」は0〜10で指定してください（現在=${v}）。`);
    }
    const c = {
...
export async function deleteCharacter(ownerId, idOrName) {
    const db = await loadDB();
...
export async function selectCharacter(sel, idOrName) {
    const db = await loadDB();
...
export async function currentCharacter(sel) {
    const s = await loadSelections();
...
export async function trySkillRoll(messageContent, sel) {
    ...
    if (band.tag === "REV")
        lines.push("★革命的成功：追加良効果フラグ");
    if (band.tag === "FUM")
        lines.push("★致命的失態：追加悪効果フラグ");

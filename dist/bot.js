...
if (process.env.GUILD_ID)
    deployCommands(process.env.GUILD_ID).catch(console.error);
client.on(Events.InteractionCreate, async (i) => {
    try {
        if (i.isChatInputCommand()) {
            ...
            if (i.commandName === "ms")
                return handleMs(i);
            if (i.commandName === "avoid")
                return handleAvoid(i);
            if (i.commandName === "board")
                return handleBoard(i);
            return;
        }
        if (i.isButton() || i.isStringSelectMenu()) {
            if (await handlePcButton(i))
                return;
            if (await handleMsButton(i))
                return;
            if (await handleBoardComponent(i))
                return;
        }
    }
    catch (e) {
